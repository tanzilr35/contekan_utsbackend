...
// File ini untuk pemanggilan database
class Admin_model extends CI_Model // pemanggilan akses dari controller supllier.php
{
    ...
    {
        return $this->db->delete($table, [$pk => $id]); // jadi si id ini dijadikan index primary key di database
    }
    ...
    public function getBarang()
    {
        ...
        $this->db->order_by('id_barang'); // order_by untuk mengurutkan berdsrkan id_barangnya3
        return $this->db->get('barang b')->result_array();
    }

    public function getBarangMasuk($limit = null, $id_barang = null, $range = null)
    {
        $this->db->select('*'); // bm = barang masuk
        $this->db->join('user u', 'bm.user_id = u.id_user'); // join itu untuk menggabungkan field tabel 1 ke field tabel lain
    ...
    {
        $this->db->join('satuan s', 'b.satuan_id=s.id_satuan'); // join berfungsi untuk buat disambung ke grafik atau untuk otomatis mengubah jumlah stok
